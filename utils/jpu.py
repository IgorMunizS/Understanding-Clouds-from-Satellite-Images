...
def ASPP(tensor):
    ...
    y_pool = KL.AveragePooling2D(pool_size=(
    ...
    y = KL.Concatenate(axis=-1)([y_pool, y_1, y_6, y_12, y_18])
...
def JPU_DeepLab(img_height=1024, img_width=1024, nclasses=19):
    ...
    x = KL.Concatenate(axis=-1)([x_a, x_b])
    x = conv_block(x, num_filters=256, kernel_size=3)
    x = conv_block(x, num_filters=256, kernel_size=3)
    h_t, w_t = x.shape.as_list()[1:3]
    scale = img_height // h_t, img_width // w_t
    x = KL.UpSampling2D(size=scale, interpolation='bilinear')(x)

    x = KL.Conv2D(nclasses, (1, 1), name='output_layer')(x)
