...
from utils.losses import dice_coef, dice_coef_loss_bce, lovasz_loss, combo_loss, lovasz_softmax_ce
...
def train(smmodel,backbone,batch_size,shape=(320,480),nfold=0,pseudo_label=None):
    ...
    for n_fold, (train_indices, val_indices) in enumerate(skf.split(mask_count_df.index, mask_count_df.hasMask)):
        ...
        if n_fold >= nfold:
            ...
            ckp = ModelCheckpoint(filepath, monitor='val_loss', verbose=1, save_best_only=True, mode='max',
                                         save_weights_only=True)
            es = EarlyStopping(monitor='val_loss', min_delta=0.0001, patience=5, verbose=1, mode='min')
            rlr = ReduceLROnPlateau(monitor='loss', factor=0.2, patience=2, verbose=1, mode='max', min_delta=0.0001)
