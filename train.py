...
from utils.losses import jaccard, sm_loss, combo_loss_init, combo_loss_ft, lovasz_loss,bce_lovasz_softmax
...
def train(smmodel,backbone,batch_size,shape=(320,480),nfold=0,pseudo_label=None):
    ...
    for n_fold, (train_indices, val_indices) in enumerate(skf.split(mask_count_df.index, mask_count_df.hasMask)):
        ...
        if n_fold >= nfold:
            ...
            model = get_model(smmodel,backbone,opt,bce_lovasz_softmax,[dice_coef])
